...
class MainScreen(MDScreen):
    ...
    # очистка полей ввода
    def clear(self):
        self.ids.vet_name.text='k'
       # self.ids.appointment_date.text = self.controller.return_last_app_date()
        #self.ids.birth_date.text = self.controller.return_birth_date()

        # установка даты рождения через календарь
    def choose_birth_date(self):
        self.date_dialog = MDDatePicker(min_year=1990, max_year=2022)
        self.date_dialog.bind(on_save=self.set_birth_date)
        print('open')
        self.date_dialog.open()
    def set_birth_date(self, instance, value, date_range):

        self.controller.set_birth(str(value))
        self.ids.birth_date_input.text = str(value)
        print(str(value))
        self.date_dialog.dismiss()


    # установка даты последнего приёма через календарь
    def choose_last_appointment_date(self):
        self.date_dialog = MDDatePicker(min_year=2000, max_year=2022)
        self.date_dialog.bind(on_save=self.set_last_appointment_date_dia)
        self.date_dialog.open()

    def set_last_appointment_date_dia(self, instance, value, date_range):
        self.controller.set_last_appointment_date(str(value))
    ...
    def show_dialog(self):
        if not self.dialog:
            self.dialog = MDDialog(
                title='Удаление',
            buttons=[
                MDFlatButton(text='click', on_release=self.close_dialog)
            ]
            )
            # table = MDDataTable(
            #     column_data=[
            ...
            #     ]
            # )
        #self.dialog.add_widget(inp)
        #self.dialog.add_widget(table)
        self.dialog.open()
